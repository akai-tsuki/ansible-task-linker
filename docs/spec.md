...
主な機能:
- `import_tasks` / `include_tasks` / `import_role` / `include_role` / `import_playbook` / `vars_files` に対するクリック遷移（DocumentLink）
...
| 項目 | 内容 |
|------|------|
| 対象ディレクティブ | `vars_files` |
| リンク対象 | シーケンスの各スカラー要素に記述されたファイルパス |
| パス解決方法 | 現在開いているYAMLファイルからの**相対パス**で解決 |
| リンク表示範囲 | ファイルパス文字列の部分のみ（クォートは除く） |
| ファイルが存在しない場合 | リンクを表示しない |
...
| 項目 | 内容 |
|------|------|
| 対象ディレクティブ | DocumentLink と同一（2.1.1 〜 2.1.4 参照） |
...
### 8.5 vars_files テストケース

| ケース | 入力 | 期待結果 |
|--------|------|----------|
| 基本ケース | `vars_files:\n  - vars/common.yml` | `vars/common.yml` にリンク |
| 複数エントリ | `vars_files:\n  - vars/a.yml\n  - vars/b.yml` | それぞれリンクあり |
| ダブルクォート | `vars_files:\n  - "vars/common.yml"` | リンクあり |
| シングルクォート | `vars_files:\n  - 'vars/common.yml'` | リンクあり |
| ファイル不存在 | `vars_files:\n  - vars/missing.yml` | リンクなし |
| Jinja2変数 | `vars_files:\n  - "{{ vars_dir }}/common.yml"` | リンクなし |

### 8.6 tasks_from テストケース
...
| 追加ディレクティブ | `include_vars` など、今回対象外のディレクティブ |
